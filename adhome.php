     ...
     <nav id="nav-bar">
      <header>Mystore</header>
        <ul>
            ...
            <li><a class="nav-link" href="#list1">Queries</a></li>
...
function userfun($fn,$ln,$usr,$no,$add){
    ...
    ?>
     ...
     <header>Customer Queries</header>
     <article>
     <table style="width:100%" >
<tr>
<th>Username</th>
...
<?php
function orderfun($fn,$ln,$prod,$price){
       $rslt = "<tr>
       <th>$fn $ln</th>
       <th>$prod</th> 
       <th>$price</th> 
    ...
    $osql = "SELECT f_name,l_name,pro_name,pro_rs from usertable,mobile,cart where c_user=username and c_id=pro_id ";
    $oretval = mysqli_query( $con, $osql);
    
    if(mysqli_num_rows($oretval)>0){
    while($row = mysqli_fetch_array($oretval)) {
            orderfun(
                    $row['f_name'],
                    $row['l_name'],
                    $row['pro_name'],
